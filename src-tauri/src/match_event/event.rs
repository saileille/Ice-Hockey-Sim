...
impl Shot {
    ...
    pub fn create_shooter_and_assisters(&mut self) {
        let players = self.event.attacking_players.get().get_skaters_in_vector();
    ...
    pub fn calculate_goal(&mut self) {
        let gk_ability = self.event.defending_players.get_goalkeeper().unwrap().ability as f64;
        let shooter_ability = self.get_shooter().ability as f64;
