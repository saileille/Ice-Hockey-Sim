...
use crate::logic::{app_data::Directories, io::{get_flag_path, load_country_names}, person::Gender, types::{CountryId, CountryNamePool, Db}};
...
impl Country {
    ...
    fn build(directories: &Directories, name: &str) -> Self {
    ...
    pub async fn build_and_save(directories: &Directories, db: &Db, name: &str) -> Self {
        let mut country = Self::build(directories, name);
