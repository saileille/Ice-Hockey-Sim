...
use crate::logic::{game::event::PlayersOnIce, person::{attribute::PersonAttribute, player::Player}, random_with_weights, team::lineup::{DefencePair, ForwardLine, LineUp}, types::{Db, PersonId}};

#[derive(Debug)]
#[derive(Default, Clone)]
pub struct LineUpCache {
    pub goalkeepers: [Option<Player>; 2],
...
impl LineUpCache {
    ...
    pub fn average_ability(&self) -> f64 {
        ...
        for gk in self.goalkeepers.iter() {
            if gk.is_some() {
                total_ability += PersonAttribute::display(
                    gk.as_ref().unwrap().ability.value
                ) as u16;
                counter += 1;
            }
        }
        ...
        for pair in self.defence_pairs.iter() {
            if pair.ld.is_some() {
                total_ability += PersonAttribute::display(
                    pair.ld.as_ref().unwrap().ability.value
                 ) as u16;
                counter += 1;
            }
            if pair.rd.is_some() {
                total_ability += PersonAttribute::display(
                    pair.rd.as_ref().unwrap().ability.value
                ) as u16;
                counter += 1;
            }
        }
        ...
        for line in self.forward_lines.iter() {
            if line.lw.is_some() {
                total_ability += PersonAttribute::display(
                    line.lw.as_ref().unwrap().ability.value
                ) as u16;
                counter += 1;
            }
            if line.c.is_some() {
                total_ability += PersonAttribute::display(
                    line.c.as_ref().unwrap().ability.value
                ) as u16;
                counter += 1;
            }
            if line.rw.is_some() {
                total_ability += PersonAttribute::display(
                    line.rw.as_ref().unwrap().ability.value
                ) as u16;
                counter += 1;
            }
        }
...
#[derive(Debug)]
#[derive(Default, Clone)]
pub struct DefencePairCache {
    pub ld: Option<Player>,
    pub rd: Option<Player>,
}
...
#[derive(Debug)]
#[derive(Default, Clone)]
pub struct ForwardLineCache {
    pub lw: Option<Player>,
    pub c: Option<Player>,
    pub rw: Option<Player>,
}
