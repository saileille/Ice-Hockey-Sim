...
use crate::logic::{person::{attribute::PersonAttribute, contract::{Contract, ContractRole}, player::{Player, position::PositionId}}, team::Team, types::{Db, PersonId, convert}};
...
impl PlayerNeed {
    ...
    pub fn get_role_of_player(&self, player: &Player) -> f64 {
        ...
        for ability in self.abilities.iter() {
            if PersonAttribute::display(player.ability.value) as f64 > *ability {
                break;
            }
            index += 1.0;
        }
    ...
    async fn evaluate_player(&self, db: &Db, player: &Player) -> f64 {
        ...
        (PersonAttribute::display(player.ability.value) as f64 - worst) * self.urgency * (1.0 / (player.person.no_of_offers(db).await + 1) as f64)
    }
}
...
impl Team {
    ...
    pub async fn send_contract_offer(&mut self, db: &Db, today: Date, person_id: PersonId, years: i32, role: ContractRole) {
        Contract::build_from_years(db, today, person_id, self, years, role, false).await;
    ...
    pub async fn create_contract_offer(&mut self, db: &Db, today: Date, player: Player) {
        let years = rand::random_range(1..=4);  // 1-4 year contract offers, just like MHM.
        self.send_contract_offer(db, today, player.person.id, years, ContractRole::Player).await;
    }
...
fn evaluate_position_needs(position: PositionId, players: Vec<Player>) -> PlayerNeed {
    ...
    need.abilities = players[0..players_in_lineup].into_iter()
    .map(|a| PersonAttribute::display(a.ability.value) as f64)
    .collect();
