...
use crate::logic::{competition::{Competition, round_robin::RoundRobin as RoundRobinFormat, season::{Season, knockout_round::{KnockoutPair, KnockoutRound as KnockoutRoundSeason}, team::TeamSeason}}, game::Game, time::dates_between, types::{Db, TeamId, convert}};
...
impl Season {
    ...
    async fn generate_match_pool(&self, db: &Db, comp: &Competition) -> Vec<[TeamId; 2]> {
        ...
        let round_robin = comp.round_robin_format(db).await.unwrap();

        let matches_in_round = round_robin.round_length(db, self).await;
        let matches_in_full_round = matches_in_round * 2;
        let mut matches = round_robin.theoretical_matches_per_team(db, self).await;
...
pub async fn assign_dates(db: &Db, matchdays: Vec<Vec<[TeamId; 2]>>, start_date: Date, end_date: Date, comp: &Competition, randomise_order: bool) {
    let mut dates = dates_between(start_date, end_date);
...
async fn build_games(db: &Db, match_pool: &[[TeamId; 2]], date: Date, comp: &Competition) {
    ...
    for matchup in match_pool {
        Game::save(db, matchup[0], matchup[1], season_id, date).await;
    }
}
...
impl KnockoutRoundSeason {
    ...
    pub async fn generate_matchdays(&self, db: &Db, comp: &Competition) -> Vec<Vec<[TeamId; 2]>> {
        let matches = comp.knockout_round_format(db).await.unwrap().maximum_games;
