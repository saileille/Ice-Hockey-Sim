...
use crate::logic::{competition::{self, Competition, Seed, comp_connection::CompConnection, knockout_round::KnockoutRound as KnockoutRoundFormat, season::ranking::RankCriteria}, game, time::{AnnualDate, AnnualWindow, dates_between}, types::{CompetitionId, Db, convert}};

// Generate a knockout competition with each round being represented as its own competition element.
pub async fn build(
    db: &Db, rng: &mut ThreadRng, today: Date,
    ...
    match_rules: Vec<game::Rules>,    // Match rules for each round. If there are more rounds than elements in this vector, the last element will be applied to remaining rounds.
...
async fn build_round(db: &Db, round_names: &[&str], match_rules: &[game::Rules], wins_required: &[u8], teams_in_rounds: &[u8], rank_criteria: &Vec<RankCriteria>, parent_comp_id: CompetitionId, round_size: u8, i: usize) -> Competition {
    ...
    let mut round = Competition {
        ...
        comp_type: competition::Type::KnockoutRound,
...
async fn set_date_boundaries(db: &Db, rng: &mut ThreadRng, rounds: &mut Vec<Competition>, season_duration: &AnnualWindow) {
    // Let's get our example dates from a year that was not a leap year.
    let (start_date, end_date) = season_duration.dates_from_start_year(1900);
    let available_days = dates_between(start_date, end_date);

    let round_durations = get_round_durations(db, rounds, convert::int::<usize, u8>(available_days.len()), rng).await;
...
async fn get_round_durations(db: &Db, rounds: &[Competition], days: u8, rng: &mut ThreadRng) -> Vec<u8> {
    let mut matches_in_rounds = Vec::new();
    for round in rounds {
        matches_in_rounds.push(round.knockout_round_maximum_games(db).await.unwrap() as f64);
    }
...
// Finalise the rounds.
async fn finalise_rounds(db: &Db, today: Date, parent_comp: &mut Competition, rounds: &mut Vec<Competition>) {
    let rounds_clone = rounds.clone();
    let mut connections = vec![0; rounds.len()];

    let teams = Vec::new();
    for (i, round) in rounds.iter_mut().enumerate() {
        round.create_new_season(db, today, &teams).await;
    ...
    parent_comp.save_type(db).await;
}
...
async fn create_connections(db: &Db, i: usize, round: &mut Competition, rounds: &[Competition], connections: &mut Vec<u8>) {
    ...
    while advancing_teams > 0 {
        ...
        let connection = CompConnection::build(round.id, first as u8, last as u8, Seed::Preserve, false);
        connection.save(db, round.id + iteration).await;
